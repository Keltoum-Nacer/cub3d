...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/17 21:49:35 by knacer            #+#    #+#             */
/*   Updated: 2025/01/17 23:00:14 by knacer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_left_right(t_data *data, int keycode)
{
	...
	if (keycode == KEY_A)
	{
		data->new_x = data->map.p.p_x - SPEED * cos(data->map.p.angle + PI / 2) - CO
			* cos(data->map.p.angle + PI / 2);
		data->new_y = data->map.p.p_y - SPEED * sin(data->map.p.angle + PI / 2) - CO
			* sin(data->map.p.angle + PI / 2);
		validate_move(data, -CO * cos(data->map.p.angle + PI / 2),
