...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/01/17 22:11:22 by knacer            #+#    #+#             */
/*   Updated: 2025/01/17 23:23:44 by sait-amm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_textures(t_data *data)
{
	...
	while (i < 7)
	{
		data->ts[i].t.image = mlx_xpm_file_to_image(data->mlx.mlx,
				names[i], &data->text.width, &data->text.height);
		...
		data->ts[i].t.image_addr = mlx_get_data_addr(data->ts[i].t.image,
				&data->ts[i].t.bits_per_pixel,
				&data->ts[i].t.line_length, &data->ts[i].t.endian);
		i++;
	}
	free(names[4]);
    free(names[5]);
